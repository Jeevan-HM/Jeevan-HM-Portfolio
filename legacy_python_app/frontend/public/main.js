    ...
    updateChatText(chatbox) {
        ...
        this.messages.slice().reverse().forEach(function (item, index) {
            ...
            if (item.name === "J.A.I.D") {
                ...
                if (typeof marked !== 'undefined' && (parsedMessage.includes('**') || parsedMessage.includes('*') || parsedMessage.includes('```') || parsedMessage.includes('#'))) {
                    parsedMessage = marked.parse(parsedMessage);
                }

                // Sanitize HTML to prevent XSS
                if (typeof DOMPurify !== 'undefined') {
                    parsedMessage = DOMPurify.sanitize(parsedMessage, { ADD_ATTR: ['target'] }); // Allow target attr for links
                }

                html += '<div class="messages__item messages__item--visitor">' + parsedMessage + '</div>';
            } else {
